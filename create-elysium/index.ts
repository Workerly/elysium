...
c.write(`Welcome to ${c.format('Elysium', ConsoleFormat.MAGENTA, ConsoleFormat.UNDERLINE)}`);
c.write('This tool will help you create a new Elysium project.');
c.newLine();
...
const answers = await prompts(
	[
		...
		},
		{
			...
			choices: [
				{ title: 'Single Module', value: 'single', description: 'A single module project' },
				{
					title: 'Multi Module',
					value: 'multi',
	...
	],
	{
		...
		onSubmit(prompt, answer) {
			if (prompt.name === 'override_path' && answer === false) {
				c.error('Project creation cancelled.');
				process.exit(1);
			}
		}
	}
